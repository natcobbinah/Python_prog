class Heap:
    ...
    def arrange(self,k):
        while k // 2 > 0: # integer floor division
            if self.heap[k] > self.heap[k // 2]: # if value at index is greater than parent value
    ...
    def maxIndex(self,k):
        ...
        if k * 2 + 1 > self.size:
            return k * 2
        # otherwise, we simply return the index of the max of the two children
    ...
    def sink(self,k):
        while k * 2 <= self.size:
            ma = self.maxIndex(k) # which of the children (left or right) to compare against

            # now we compare parent with the child to see if a swap should occur
            if self.heap[k] < self.heap[ma]:
